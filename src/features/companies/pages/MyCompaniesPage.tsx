...
export function MyCompaniesPage() {
    ...
    try {
      ...
      if (err.response?.status === 401) {
        ...
        const errorMessage = Array.isArray(errorDetail)
          ...
          ? errorDetail
          : t('pages.companies.failedToLoad');
  ...
  const handleDeleteCompany = async (companyId: number) => {
    if (!window.confirm(t('pages.companies.confirmDelete'))) {
      return;
    }
    ...
    try {
      ...
      const errorMessage = typeof errorDetail === 'string'
        ? errorDetail
        : t('pages.companies.failedToDelete');
      alert(errorMessage);
    } finally {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-blue-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-4 text-gray-600">{t('pages.companies.loading')}</p>
        </div>
      </div>
    );
  }
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div className="flex-1">
              <div className="flex items-center space-x-2 sm:space-x-3 mb-1 sm:mb-2">
                ...
                <h1 className="text-2xl sm:text-3xl font-bold text-gray-900">{t('pages.companies.title')}</h1>
              </div>
            ...
            >
              ...
              <span>{t('pages.companies.createFirst')}</span>
            </Link>
          </div>
        </div>
        ...
        {companies.length === 0 ? (
          <div className="bg-white rounded-xl shadow-md p-12 text-center">
            ...
            <p className="text-gray-500 mb-4">
              {t('pages.companies.noCompaniesDesc')}
            </p>
            <Link
              to="/companies/create"
                ...
                <div className="space-y-1.5 sm:space-y-2 mb-3 sm:mb-4 flex-1">
                  ...
                  {company.size && (
                    <div className="flex items-center text-xs sm:text-sm text-gray-600">
                      ...
                      <span>{company.size} {t('pages.companies.employeesCount')}</span>
                    </div>
                  )}
                  {company.website && (
                    <div className="flex items-center text-xs sm:text-sm text-gray-600">
